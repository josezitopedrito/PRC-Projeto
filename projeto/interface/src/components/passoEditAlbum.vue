            ...
            <v-container>
                ...
                >
                  <template class="tile" v-slot:item.options="{ item }">
                    <v-btn icon @click="editAlbum(item.id.split('#')[1])" >
                      <v-icon
                        small
                        class="mr-2"
                      >
    ...
    editAlbum: async function(idAlbum){
        try{
            ...
            for(let i=0;i<response.data.band.length;i++){
                this.album.albumGroup.push(response.data.band[i].band.split('#')[1])
            }
            for(let i=0;i<response.data.recordLabel.length;i++){
                this.album.albumLabel.push(response.data.recordLabel[i].rlabel.split('#')[1])
            }
            for(let i=0;i<response.data.producer.length;i++){
                this.album.albumProducer.push(response.data.producer[i].producer.split('#')[1])
            }
            this.save()
        }catch(e){
            console.log(e)
