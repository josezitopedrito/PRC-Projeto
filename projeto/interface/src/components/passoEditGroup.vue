            ...
            <v-container>
                ...
                >
                  <template class="tile" v-slot:item.options="{ item }">
                    <v-btn icon @click="editGroup(item.id.split('#')[1])">
                      <v-icon
                        small
                        class="mr-2"
                      >
        ...
        <v-form ref="form2" method="post" enctype="multipart/form-data" v-if="position != 'first' && position != 'second'">
            ...
            <v-container>
              ...
              <v-btn ref="skip" @click="saveSkipGenre();" class="orange white--text" v-if="position == 'groupGenre'">Saltar</v-btn>
                <div v-if="position == 'albumGroup' && album.albumGroup.length>0" style="overflow-y: scroll;height: 100px;width: 800px;">
                ...
                <div v-if="position == 'groupArtist' && artist.groupArtist.length>0" style="overflow-y: scroll;height: 100px;width: 800px;">
                  <h3>Groups this artist has been a part of:</h3>
                  <ul v-for="n in artist.groupArtist" :key="n">
                      <li>{{n}}<v-btn depressed @click="deleteItem(n)">Eliminar</v-btn></li>
                  </ul>
                </div>
                <div v-if="position == 'groupGenre' && genre.groupGenre.length>0" style="overflow-y: scroll;height: 100px;width: 800px;">
...
export default {
  ...
  methods:{
    ...
    editGroup: async function(idGroup){
        try{
            ...
            this.group.homepage = response.data.band[0].hp
            this.group.hometown = response.data.band[0].ht
            this.group.groupInfo = response.data.band[0].abs
            for(let i=0;i<response.data.album.length;i++){
                this.group.albumGroup.push(response.data.album[i].album.split('#')[1])
            }
            for(let i=0;i<response.data.artist.length;i++){
                this.group.groupArtist.push(response.data.artist[i].artist.split('#')[1])
            }
